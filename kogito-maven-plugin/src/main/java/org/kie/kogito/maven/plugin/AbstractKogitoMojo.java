...
import org.kie.kogito.codegen.manager.util.RunDebugUtil;
import org.kie.kogito.maven.plugin.util.MojoUtil;

public abstract class AbstractKogitoMojo extends AbstractMojo {
    ...
    protected String mavenCompilerSourceJavaVersion;

    @Parameter(defaultValue = "17", property = "maven.compiler.target")
    protected String mavenCompilerTargetJavaVersion;
    ...
    public void buildProject() throws MojoExecutionException {
        try {
            Set<URL> projectFilesUrl = MojoUtil.getProjectFiles(mavenProject, null);
            BuilderManager.BuildInfo buildInfo = new BuilderManager.BuildInfo(projectFilesUrl,
                    ...
                    mavenCompilerSourceJavaVersion,
                    mavenCompilerTargetJavaVersion,
                    jsonSchemaVersion,
                    generatePartial,
                    persistence,
                    mavenProject.getRuntimeClasspathElements(),
                    discoverFramework());
            MojoUtil.hasClassOnClasspath(mavenProject, "");
    ...
    void runProject() throws MojoExecutionException {
        try {
            RunDebugUtil.runProject(mavenProject.getBasedir(), mavenProject.getBuild().getDirectory(), mavenProject.getBuild().getFinalName() + ".jar");
        } catch (IOException e) {
            throw new MojoExecutionException("Error building project", e);
        }
    }
    ...
    boolean isProjectBuild() {
        return projectBuildOutputDirectory.exists() && !Objects.isNull(projectBuildOutputDirectory.listFiles()) && Objects.requireNonNull(projectBuildOutputDirectory.listFiles()).length > 0;
    }

    CodeGenManagerUtil.Framework discoverFramework() {
        if (MojoUtil.hasDependency(mavenProject, "quarkus")) {
            return CodeGenManagerUtil.Framework.QUARKUS;
        }

        if (MojoUtil.hasDependency(mavenProject, "spring")) {
